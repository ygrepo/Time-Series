...
the handout. The initial distribution of the initial state $\vect{z}_1$, $\vect{\pi}$ is implemented by an E-M step. 
The initial values of the means are provided by KMeans and updated by an expectation-maximization algorithm.
...
The result for $K=3$, are plotted below with the top plot showing the log loss of the data, the middle plot the inference obtained with our model and the bottom the plot using the means and covariances
...
The performance of our model compared to hmmlearn Gaussian smoother,  are similar, the GaussianHMM model covering more of the data along the x-axis whereas our model cover  the
